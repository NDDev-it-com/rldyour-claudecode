...
Reviewer output uses a **file-first transport contract** (full text in `plugins/rldyour-flow/references/reviewer-protocol.md`, section "Output Transport") to work around the Claude Code 2.0.77+ `task.output` regression (Anthropic issues `#16789`, `#20531`, `#23463`, closed as "not planned"). Orchestrator (`ry-start` / `ry-review`) generates `run_id = <UTC-ISO-compact>-<git-short-sha>`, computes `report_dir = .serena/reviews/<run_id>/` (gitignored runtime artefact alongside `.serena/cache/` and `.serena/diagnostics/`), and injects both into every reviewer prompt. Each reviewer writes the full long-form report to `<report_dir>/<reviewer-name>.md` via `Bash` and returns a compact summary ≤ 4 KB (Report path, severity counts, all findings as one-liners capped at 30). 6 tracks × ≤ 4 KB = ≤ 24 KB injected into parent context, structurally preventing the overflow class.
