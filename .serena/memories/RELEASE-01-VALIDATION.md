...
- Current product/config version is `1.1.13`; `VERSION`, `package.json`,
  `pyproject.toml`, and `CHANGELOG.md` are the source of truth for the
  adapter-local SemVer state.
- Release `1.1.13` keeps the Claude Code runtime baseline at `2.1.158`,
...
## Historical evidence
- Release `1.1.13` adopted Claude Code `2.1.153` and the refreshed common MCP
...
- Release `1.1.13` hardens the rldyour-flow Stop post-task sync gate: direct
...
- Release `1.1.13` aligns active repository descriptions with the root
  ...
  validation.
- Release `1.1.13` adopts the Claude Code `2.1.154` baseline required for
...
- Release `1.1.13` refreshes the local `github-mcp-server` host-binary pin to
  `1.1.0` in `config/mcp-runtime-versions.env`, matching the installed
  Homebrew binary and GitHub MCP Server release.
- Release `1.1.13` pinned Claude Code `2.1.156`, recorded the Opus 4.8
...
- Release `1.1.13` preserves the slash-command thin-wrapper invariant by
  restoring the required `ry-start` skill delegation phrase in the
  workflow-aware `/rldyour-flow:ry-start` shim.
- Release `1.1.13` was superseded by `1.1.5` after the GitHub release workflow
