...
| Opus 4.8 thinking-block API hotfix | 2.1.156 | Adopted | Preserve the hotfix introduced in `2.1.156`; pin package/runtime surfaces to Claude Code `2.1.160` and require installed-runtime smoke to report `claude --version` `2.1.160` or newer before declaring Opus 4.8 release readiness. | `scripts/validate_claude_surface_adoption.py` |
...
| Claude Code 2.1.160 sensitive-write and background-session hardening | 2.1.160 | Adopted | Official changelog adds prompts before writing shell startup files and execution-granting build config files, lets single-file `grep` satisfy read-before-edit, and fixes background-agent/session regressions. Keep first-party plugin config unchanged; update the runtime pin and use installed-runtime smoke to prove the local binary. | `scripts/validate_runtime_baselines.py`; `scripts/validate_model_baselines.py`; root `scripts/smoke_claude_ry_start_workflow.py` |
...
surface, and every pinned `2.1.160` runtime surface that affects this
