...
| Dynamic workflows and `/workflows` | 2.1.154 | Adopted / hybrid | Keep `/rldyour-flow:ry-start` as the stable plugin command and route workflow-capable Claude Code sessions to the project `ry-start-workflow` boundary. `plugins/rldyour-flow/skills/ry-start/SKILL.md` remains the human-readable fallback because official docs expose saved-workflow creation through the interactive `/workflows` view, not a stable file-format generator. | `scripts/validate_claude_surface_adoption.py` |
