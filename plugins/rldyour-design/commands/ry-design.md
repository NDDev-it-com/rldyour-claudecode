...
5. **Components** — shadcn/ui MCP via `mcp__shadcn__*` (Browse / Search / Install) for primitives and registry blocks; ReactBits.dev only for purposeful motion or interactive effects.
6. **Implementation** — Serena-first inspection (`get_symbols_overview` → `find_symbol`), then code; never paste generated code blindly.
7. **Validation** — `design-validation` + `rldyour-browser` (Playwright `mcp__playwright__*` + Chrome DevTools `mcp__chrome-devtools__*`) for pixel-perfect, responsive, runtime, business-logic checks.
