...
- `1` subagent: `ry-explore` (`model: opus[1m]`, `effort: max`, `maxTurns: 90`, `color: cyan`, `disallowedTools: [Edit, Write, NotebookEdit]`) — runs in forked context with deep MCP-first research workflow (Context7 → DeepWiki → Grep → Web validation → cross-validation → synthesis).
