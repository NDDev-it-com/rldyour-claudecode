...
8. Detect semantic entropy: duplicated docs, stale pins, conflicting instructions, dead config, unclear source-of-truth, missing ADR/CONTEXT/FUTURE facts, broken validator contracts, and natural-language policy overrides that are not materialized in project policy JSON.
...
11. Apply technical-only repairs using existing project patterns and native adapter surfaces. If the user explicitly states repository policy and no policy file exists, write `.rldyour/project-policy.json` or `.rldyour/project-policy.local.json` only from that explicit instruction; do not infer ownership/destructive permissions from chat tone.
