...
RUNTIME_RE='^(\.serena/cache(/|$)|\.serena/\.gitignore$|\.serena/project\.local\.yml$|\.serena/\.(sync_marker|serena_sync_state\.json|auto_sync_head|active_workflow_intent\.json|dirty_stop_ack|flow_sync_marker|flow_post_task_state\.json|flow_blocker_ack\.json|stop_lifecycle_timeout_marker)$|\.playwright-mcp(/|$)|browser(/|$)|\.env$|\.env\.[^/]+$)'
