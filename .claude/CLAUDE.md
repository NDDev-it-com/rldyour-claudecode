...
Reviewer output transport is **file-first** since rldyour-flow `0.2.1`: each reviewer writes the full long-form report to `.serena/reviews/<run_id>/<reviewer-name>.md` via `Bash` and returns only a ≤ 4 KB compact summary (Report path + severity counts + one-liner findings, cap 30). The orchestrator (`ry-start` / `ry-review` skill body) generates one `run_id = <UTC-ISO-compact>-<git-short-sha>` per wave, injects `report_dir` into every reviewer prompt, reads per-reviewer report files via `Read` only for critical/high findings, and writes `<report_dir>/_summary.md` as the consolidated wave artefact. Full contract in `plugins/rldyour-flow/references/reviewer-protocol.md` "Output Transport". Rationale: Claude Code 2.0.77+ regression (Anthropic `#16789`, `#20531`, `#23463`, all closed as "not planned") can deliver 200-500 KB JSONL transcript per subagent and overflow parent context; 6 reviewers × ≤ 4 KB = ≤ 24 KB structurally prevents the failure class. Read-only invariant preserved (`Edit`/`Write`/`NotebookEdit` still absent; `Bash` writes are scoped to `report_dir` only).
